...
df1 = read.csv(file = file.path(Proj.Home,"Data","4_Data.csv"), header = TRUE) %>%
  mutate(Virus_Cell = str_c(Virus," in ", Cell))

Baseline.df = df1 %>% 
  filter(MOI == 0) %>%
  group_by(Virus_Cell) %>%
  dplyr::summarise(Base = mean(Copy))

df2 = right_join(df1,Baseline.df) %>%
  mutate(Fold = Copy / Base)

df3 = df2 %>% 
  group_by(Virus_Cell,MOI) %>%
  dplyr::summarise(Fold = mean(Fold)) %>%
  filter(Fold == max(Fold))

df4 = df2 %>%
  mutate(MOI = factor(MOI)) %>%
  group_by(Virus, Cell, Virus_Cell, MOI) %>%
  dplyr::summarise(Fold.Avg = mean(Fold),
                   Fold.se = sd(Fold) / sqrt(length(Fold)))

Base.Plot = 
  ggplot() +
  scale_color_manual(values = c("GFHK99 in MDCK" = "skyblue",
                                "GFHK99 in DF-1" = "deeppink3",
                                "GFHK99 in HTBE" = "steelblue",
                                "NL09 in MDCK" = "goldenrod",
                                "NL09 in HTBE" = "purple",
                                "MaMN99 in MDCK" = "green3",
                                "MaMN99 in DE" = "firebrick3")) +
  scale_fill_manual(values = c("GFHK99 in MDCK" = "skyblue",
                               "GFHK99 in DF-1" = "deeppink3",
                               "GFHK99 in HTBE" = "steelblue",
                               "NL09 in MDCK" = "goldenrod",
...
Base.Plot +
  ...
  geom_col(data = df4 %>% filter(MOI != "0", Virus == "GFHK99", Cell != "HTBE"),
           position = position_dodge(),
           color = "black",
           aes(x = MOI,
               y = Fold.Avg,
               fill = Virus_Cell)) +
  geom_errorbar(data = df4 %>% filter(MOI != "0", Virus == "GFHK99", Cell != "HTBE"),
                aes(x = MOI,
                ...
                width = 0.9,
                lwd = 0.8) +
  geom_jitter(data = df2 %>% filter(MOI != "0", Virus == "GFHK99", Cell != "HTBE"),
           aes(x = MOI %>% factor,
               y = Fold,
               fill = Virus_Cell),
           color = "black",
...
ggsave(file = "Plots/4A_Help_GFHK99_Bar.pdf",
       height = 5,
       width = 5,
       units = "in")
...
Base.Plot +
  geom_hline(lty = 2, yintercept = 1) +
  geom_col(data = df4 %>% filter(MOI != "0", Virus == "NL09", Cell != "HTBE"),
           position = position_dodge(),
           color = "black",
           aes(x = MOI,
               y = Fold.Avg,
               fill = Virus_Cell)) +
  geom_errorbar(data = df4 %>% filter(MOI != "0", Virus == "NL09", Cell != "HTBE"),
                aes(x = MOI,
                    ymin = Fold.Avg - Fold.se,
                    ymax = Fold.Avg + Fold.se),
                ...
                width = 0.4,
                lwd = 0.8) +
  geom_jitter(data = df2 %>% filter(MOI != "0", Virus == "NL09", Cell != "HTBE"),
              aes(x = MOI %>% factor,
                  y = Fold,
                  fill = Virus_Cell),
              color = "black",
              ...
              stroke = 1,
              pch = 21) +
  scale_y_log10(breaks = c(.001, 0.01, .1, 1, 10, 100)) +
  labs(x = "Helper virus MOI (PFU/cell)",
       y = "Fold change relative to WT only") +
  coord_cartesian(ylim = c(.001,1))

ggsave(file = "Plots/4C_Help_NL09_Bar.pdf",
       height = 5,
       width = 5,
       units = "in")
...
Base.Plot +
  geom_hline(lty = 2, yintercept = 1) +
  geom_jitter(data = df2 %>% filter(MOI > 0,
                                    Fold > 0,
                                    Virus == "MaMN99"),
              aes(x = MOI,
                  y = Fold,
                  color = Virus_Cell),
              width = 0.05) +
  geom_smooth(data = df2 %>% filter(MOI > 0, 
                                    Cell == "DE",
                                    Virus == "MaMN99"),
              aes(x = MOI,
                  y = Fold,
                  ...
                  fill = Virus_Cell)) +
  scale_x_log10() +
  scale_y_log10(breaks = c(100,1,.01)) +
  labs(x = "Helper virus MOI (PFU/cell)",
       y = "Fold change relative to WT only") +
  coord_cartesian(xlim = c(.01,5),
                  ylim = c(1e-4,3))
...
Base.Plot +
  ...
  geom_col(data = df4 %>%
             ...
             filter(MOI != "0", Virus == "MaMN99", Cell != "HTBE"),
           position = position_dodge(),
           color = "black",
           aes(x = MOI,
               y = Fold.Avg,
  ...
  labs(x = "Helper virus MOI (PFU/cell)",
       y = "Fold change relative to WT only") +
  coord_cartesian(ylim = c(5e-5,1))

ggsave(file = "Plots/4B_Help_MaMN99_Bar.pdf",
       height = 5,
       width = 5,
       units = "in")
