...
# 2A: Fold change amplification vs. MOI ----
...
ggsave('Plots/2A_Amplification_by_MOI_Bar.pdf',
       width = 4,
       height = 4,
       unit = "in")

Legend.2A = Base.Plot + geom_bar(data = Norm.df %>% mutate(MOI = (10^MOI) %>% round(2) %>% factor,
                                               Virus_Cell = Virus_Cell %>% factor(levels = c("WF10 in DF-1","WF10 in MDCK","MN99 in MDCK","NL09 in MDCK"))) %>%
                       group_by(MOI,Virus_Cell) %>%
                       dplyr::summarise(Amp_Norm_Mean = mean(Amp_Norm)),
                     aes(x = MOI,
...
ggsave(g_legend(Legend.2A),
       file = "Plots/Legend_2A.pdf",
       height = 3,
       width = 2,
       units = "in")
...
ggsave('Plots/2B_Burst_by_MOI_High_Bar.pdf',
       width = 4,
       height = 4,
       unit = "in")
...
ggsave(file = "Plots/5A_WT_vs_Helper.pdf",
       height = 5,
       width = 5,
       units = "in")
...
ggsave(file = "Plots/5B_Segments.pdf",
       height = 5,
       width = 5,
       units = "in")
...
SCA = read.csv(file.path(Proj.Home,"Data","6A_Data.csv")) %>%
  na.omit %>%
...
Exp.Pp = read.csv(file.path(Proj.Home,"Data","6A_Data.csv")) %>%
  na.omit %>%
  mutate(Helper = PB2.Helper * PB1.Helper,
         WT.Segments = PB2 + PB1 + PA + HA + NP + NA. + M + NS,
         WT = sign(WT.Segments)) %>%
...
ggsave(file = "Plots/Misc6_Pp_v_Rep.pdf",
       width = 6,
       height = 5,
       unit = "in")
...
write.csv(Exp.Pp.2 %>% dplyr::select(Rep:NS),
          file = file.path(Proj.Home,"Data","6B_Pp_Data.csv"),
          row.names = FALSE)
...
ggsave(file = "Plots/6A_Pp_v_Segment.pdf",
       width = 5,
       height = 5,
       unit = "in")
...
write.csv(Cell.Summary %>% dplyr::select(Rep, MOI, Expressing.HA, Reassortant.Percent), file = file.path(Proj.Home,"Data","6B_Sim_Data.csv"), row.names = FALSE)
...
Cell.Summary = read.csv(file = file.path(Proj.Home,"Data","6B_Sim_Data.csv"), header = TRUE) %>% 
  na.omit
PP1_Reassortment = read.csv(file = file.path(Proj.Home,"Data","PP1_Reassortment.csv"), header = TRUE)
...
ggsave(file = "Plots/6B_Predicted_GFHK99_Reassortment.pdf",
       dpi = 300,
       width = 5,
       height = 5,
       unit = "in")
